...
- [账户和交易接口](#账户和交易接口)
	...
	- [更新用户MMP (USER_DATA)](#更新用户mmp-user_data)
	- [获取用户MMP (USER_DATA)](#获取用户mmp-user_data)
	- [删除用户MMP (USER_DATA)](#删除用户mmp-user_data)
	- [重置用户MMP (USER_DATA)](#重置用户mmp-user_data)		
	- [策略下单 (TRADE)](#策略下单-trade)
	- [更新策略订单 (TRADE)](#更新策略订单-trade)
	- [查询策略当前挂单 (USER_DATA)](#查询策略当前挂单-user_data)
	- [查询策略历史订单 (USER_DATA)](#查询策略历史订单-user_data)
	- [绑定子账户 (USER_DATA)](#绑定子账户-user_data)		
	- [创建子账户 (TRADE)](#创建子账户-trade)		
	- [查询子账户列表 (USER_DATA)](#查询子账户列表-user_data)		
	- [更新子账户 (TRADE)](#更新子账户-trade)		
...
- [Websocket 账户信息推送](#websocket-账户信息推送)
	...
	- [Event: Public Announcement](#event-public-announcement)
	- [Event: Direct Announcement](#event-direct-announcement)
- [错误代码](#错误代码)
...
[
	{
		"orderId": 20072994001, // 系统订单号
		"symbol": "BTCUSDT", // 交易对
		"status": "NEW", // 订单状态
		"clientOrderId": "myBuyOrder001", // 用户自定义的订单号
		"price": "65000.0", // 委托价格
		"avgPrice": "0.00000", // 平均成交价
...
名称              |  类型   | 是否必需   | 描述
---------------- | ------- | -------- | ----
batchOrders |	LIST | 	YES |	订单列表，最多支持5个订单（做市商白名单账户最多支持10个）
...
* orderId 与 origClientOrderId 每个订单至少发送一个，同时发送则以 orderId 为准
* quantity 与 price 每个订单均必须发送
* 每个订单的参数规则与「修改订单」一致——仅支持 `LIMIT` 类型订单，受 `PRICE_FILTER` / `PERCENT_FILTER` / `LOT_SIZE` 限制，且同一订单修改次数最多10000次
* 批量修改采取并发处理，不保证撮合顺序，但返回结果顺序始终与请求订单列表顺序一致
* 非原子操作——批量中的每个订单独立成功或失败，某个订单修改失败不会撤销该订单，也不影响批量中的其他订单
* 超出批量数量限制时返回：`"Illegal characters found in parameter 'batchOrders'; legal range is '1 to 5'."`（做市商白名单账户为 `'1 to 10'`）
...
* `signature` 必须使用**母账户钱包私钥**签名，不可使用signer私钥替代
...
| nonce | LONG | YES | 微秒级时间戳，用于防重放攻击 |
| user | STRING | NO | 签名账户的钱包地址（通常为母账户地址；子账户向母账户划转时为子账户地址）。可选——`user` 与 `signer` 二选一传入即可 |
| signer | STRING | NO | `user` 对应的代理钱包地址，通过 `registerAndApproveAgent`/`approveAgent` 注册并授权。可选——`user` 与 `signer` 二选一传入即可 |
| fromAccountAddress | STRING | NO | 转出钱包地址。当转出账户与 `user` 不一致时必传（如子→子划转，或以第三方身份发起的母→子划转） |
| signature | STRING | YES | 对消息体的签名，使用实际传入账户的私钥签名：传 `user` 则使用 `user` 的私钥签名，传 `signer` 则使用该代理的私钥签名（见下方签名说明） |
...
|------|------|------|------|------|------
| 母账户转子账户 | *(可不传——母账户地址)* | *(可不传——母账户的代理)* | 母账户地址| 子账户地址 | 使用母账户（或其代理）私钥
| 子账户转母账户 | *(可不传——母账户地址)* | *(可不传——母账户的代理)* | 子账户地址| 母账户地址 | 使用母账户（或其代理）私钥
| 子账户转子账户 | *(可不传——母账户地址)* | *(可不传——母账户的代理)* | 子账户地址A| 子账户地址B | 使用母账户（或其代理）私钥
| 子账户转母账户 | *(可不传——子账户地址)* | *(可不传——子账户的代理)* | 子账户地址| 母账户地址 | 使用子账户（或其代理）私钥
| 子账户转子账户 | *(可不传——子账户A地址)* | *(可不传——子账户A的代理)* | 子账户地址A| 子账户地址B | 使用子账户（或其代理）私钥

> `user` 与 `signer` 均为可选参数，二选一传入即可（`signer` 必须是该 `user` 已授权的代理钱包地址）。签名必须使用实际传入账户的私钥：传 `user` 则用 `user` 的私钥签名，传 `signer` 则用该代理的私钥签名。
...
使用实际传入账户的私钥对以下消息体签名：传 `user` 则使用 **`user` 账户的钱包私钥**签名；传 `signer` 则使用**该代理的钱包私钥**签名：
...
* `user` 与 `signer` 均为可选参数，二选一传入即可，只需其中一个字段即可识别签名账户。
* `signature` **必须使用实际传入账户的私钥签名**：传 `user` 则使用 `user` 的钱包私钥签名，传 `signer` 则使用该代理的钱包私钥签名。
* `signer` 必须是 `user` 已通过 `POST /fapi/v3/registerAndApproveAgent` 或 `approveAgent` 注册并获得 `canSpotTrade` 或 `canPerpTrade` 权限的代理钱包地址。
...
* 请记录返回的 `batchId`，以便通过 [用户资产迁移历史](#用户资产迁移历史-user_data) 接口查询迁移状态。
...
| 名称 | 类型 | 是否必需 | 描述 |
|------|------|---------|------|
| batchId | STRING | YES | 调用 [用户资产迁移](#用户资产迁移-withdraw) 接口时返回的批次 ID |
| nonce | LONG | YES | 微秒级时间戳，用于防重放攻击 |
