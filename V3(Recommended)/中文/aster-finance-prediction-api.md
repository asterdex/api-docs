# 基本信息
## API 基本信息
* 本篇列出接口的baseurl: **https://papi.asterdex.com**
* 所有接口的响应都是 JSON 格式。
  ...
  "predictionEvents": [
    {
      ...
      "markets": [                                            // 各结果的预测市场列表
        ...
        {
          "marketName": "EVENT4_ARGENTINA_WIN",               // 预测市场名称
          "symbols": [
          ...
          "openPrice": null,                                  // 开盘价（结算前为 null）
          "endPrice": null,                                   // 结算价（结算前为 null）
          "winAsset": "EVENT4_ARGENTINA_WIN_N",               // 获胜资产：Y 代表 YES 胜，N 代表 NO 胜
          "settleTime": 0,                                    // 结算时间（0 = 未结算）
          "settleHash": null                                  // 链上结算哈希（未结算时为 null）
        }
      ]
...
```javascript
{
  "symbol": "BTC_UP_DOWN_5M_1778483280_YUSDT", // 交易对
  "orderId": 28, // 系统的订单ID
...
* 下`MARKET` `SELL`市价单，用户通过`QUANTITY`控制想用市价单卖出的基础资产数量。
  * 比如在`BTC_UP_DOWN_5M_1778483280_YUSDT`交易对上下一个`MARKET` `SELL`市价单, 通过`QUANTITY`让用户指明想卖出多少BTC。
* 下`MARKET` `BUY`的市价单，用户使用 `quoteOrderQty` 控制想用市价单买入的报价资产数量，`QUANTITY`将由系统根据市场流动性计算出来。
  * 比如在`BTC_UP_DOWN_5M_1778483280_YUSDT`交易对上下一个`MARKET` `BUY`市价单, 通过`quoteOrderQty`让用户选择想使用多少USDT买入BTC。
...
{
    "clientOrderId": "xxx",                        // 客户自定义订单ID
    "qty": "10.00000000",                          // 每个结果代币获得的数量
    "counterparties": [                            // 结果代币及其价格列表
        {
            "symbol": "EVENT4_ALGERIA_WIN_YUSDT",  // 结果代币交易对
            "price": "0.15000000"                  // 该结果代币最新价格
        },
        {
    ...
    "sourceNoLastPrice": "0.85000000"              // 来源交易对 NO 代币的最新价格
}
```
...
{
    "clientOrderId": "xxx",                        // 客户自定义订单ID
    "qty": "10.00000000",                          // 返还的报价资产（USDT）总量
    "counterparties": [                            // 消耗的结果代币及其价格列表
        {
            "symbol": "EVENT4_ALGERIA_WIN_YUSDT",  // 消耗的结果代币交易对
            "price": "0.15000000"                  // 该结果代币最新价格
        },
        {
            "symbol": "EVENT4_ARGENTINA_WIN_YUSDT",
...
* 本篇所列出的所有wss接口的baseurl为: **wss://pstream.asterdex.com**
* Streams有单一原始 stream 或组合 stream
* 单一原始 streams 格式为 **/ws/\<streamName\>**
* 组合streams的URL格式为 **/stream?streams=\<streamName1\>/\<streamName2\>/\<streamName3\>**
* 订阅组合streams时，事件payload会以这样的格式封装: **{"stream":"\<streamName\>","data":\<rawPayload\>}**
* stream名称中所有交易对均为 **小写**
* 每个到 **pstream.asterdex.com** 的链接有效期不超过24小时，请妥善处理断线重连。
...
* **请求**
  	{    
    	...
    	[   
      	...
      	"btc_up_dowm_5m_1778483280_yusdt@depth"     
    	],    
    	"id": 1   
  	}
...
```javascript
{
  "e": "aggTrade",  // 事件类型
  "E": 123456789,   // 事件时间
  "s": "BTC_UP_DOWN_5M_1778483280_YUSDT",    // 交易对
  "a": 12345,       // 归集交易ID
  "p": "0.001",     // 成交价格
  "q": "100",       // 成交数量
  "f": 100,         // 被归集的首个交易ID
...
1. 订阅 **wss://pstream.asterdex.com/ws/btc_up_dowm_5m_1778483280_yusdt@depth**
2. 开始缓存收到的更新。同一个价位，后收到的更新覆盖前面的。
3. 访问Rest接口 **https://papi.asterdex.com/api/v3/depth?symbol=BTC_UP_DOWN_5M_1778483280_YUSDT&limit=1000** 获得一个1000档的深度快照
...
* 本篇所列出API接口的base url : **https://papi.asterdex.com**
...
* websocket接口的baseurl: **wss://pstream.asterdex.com**
